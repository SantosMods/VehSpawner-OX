...
local function isSpotClear(coords)
    ...
    for _,veh in ipairs(GetGamePool('CVehicle')) do
        if #(coords - GetEntityCoords(veh)) < r then return false end
    end
    for _,p in ipairs(GetGamePool('CPed')) do
        if not IsPedAPlayer(p) and #(coords - GetEntityCoords(p)) < r then return false end
    end
    return true
end
...
RegisterNetEvent('vehspawner:spawn', function(data)
    ...
    for _,s in ipairs(Config.Stations) do
        if s.name == data.station then st = s break end
    end
    ...
    for _,p in ipairs(st.spawns) do
        if isSpotClear(vector3(p.x,p.y,p.z)) then spawnPt = p break end
    end
    ...
    if not v then DoScreenFadeIn(400) return end

    SetVehicleOnGroundProperly(v)
    SetVehicleDirtLevel(v,0.0)
    lastVehicle = v

    exports.ox_vehiclekeys:giveKeys(v)
